...
public class JwtAuthenticationFilter implements GlobalFilter, Ordered {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        if (path.startsWith("/api/items") || path.startsWith("/api/inventory") || path.startsWith("/api/notifications") || path.startsWith("/api/logs")) {
            ...
            return authServiceClient.validateTokenAndGetUserInfo(token)
                    .flatMap(userInfo -> {
                        ...
                        // For POST, PUT, DELETE requests, and any request to audit-log, check if user has admin role
                        if (request.getMethod().name().equals("POST") || request.getMethod().name().equals("PUT") || request.getMethod().name().equals("DELETE") || (request.getMethod().name().equals("GET") && path.startsWith("/api/logs"))) {
